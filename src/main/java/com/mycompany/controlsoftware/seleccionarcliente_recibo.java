    ...
    private void ProductosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ProductosMouseClicked
        ...
        if (evt.getClickCount() == 2 && !evt.isConsumed()) {
            ...
            for (int i = 0; i < facts.length; i++) {
                ...
                fila[0] = "" + facts[i];
                fila[1] = bd.valor("select numerofactura from factura where idfactura=" + facts[i] + " and cia=" + ControlSoftware.cia);
                fila[2] = bd.valor("select saldo from factura where idfactura=" + facts[i] + " and cia=" + ControlSoftware.cia);
                fila[3] = "0.00";
                fila[4] = bd.valor("select saldo from factura where idfactura=" + facts[i] + " and cia=" + ControlSoftware.cia);
                if (Double.parseDouble("" + bd.valor("select datediff(now(),fechafactura) from factura where idfactura=" + facts[i] + " and cia=" + ControlSoftware.cia)) > Double.parseDouble(bd.valor("select diaslimite from clientes where idcliente=" + u + " and cia=" + ControlSoftware.cia))) {
                    if (bd.versiesta("select * from company_configuraciones inner join configuraciones on company_configuraciones.configuracionid=configuraciones.id inner join company on company.id_company=company_configuraciones.companyid where company_configuraciones.companyid=" + ControlSoftware.cia + " and company_configuraciones.configuracionid=" + "5")) {
                        fila[5] = new BigDecimal(fila[2].toString()).multiply(new BigDecimal(0.05)).setScale(2, RoundingMode.HALF_UP);
                    } else {
                        fila[5] = "0.00";
                    }
