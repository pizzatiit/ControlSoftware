 ...
 * @author Usuario
 */
public class mantenimiento_cuentas extends javax.swing.JInternalFrame {
    ...
    public mantenimiento_cuentas() {
        ...
        initComponents();
        bd.LlenarTabla("select C.ID_CUENTA AS COD, C.N_CUENTA AS CUENTA, B.NOMBRE AS BANCO from CUENTA C INNER JOIN BANCO B ON C.ID_BANCO=B.IDBANCO where C.cia=" + ControlSoftware.cia, Productos);
    ...
    private void initComponents() {
        ...
        jLabel5.setText("MANTENIMIENTO CUENTAS");
    ...
    String perm = "99";
        if (!bd.versiesta("select * from role_usuario inner join role on role.id=role_usuario.roleid inner join usuario on usuario.cod_user=role_usuario.usuarioid inner join role_permiso on role_permiso.roleid=role.id inner join permisos on permisos.id=role_permiso.permisoid where permisos.id=" + perm + " and role_usuario.cia=" + ControlSoftware.cia + " and usuario.cod_user=" + ControlSoftware.us)) {
            JOptionPane.showMessageDialog(null, "Usted no tiene acceso a crear cuentas.");
            return;
        }
        i_cuenta l = new i_cuenta();
...
String perm = "100";
        if (!bd.versiesta("select * from role_usuario inner join role on role.id=role_usuario.roleid inner join usuario on usuario.cod_user=role_usuario.usuarioid inner join role_permiso on role_permiso.roleid=role.id inner join permisos on permisos.id=role_permiso.permisoid where permisos.id=" + perm + " and role_usuario.cia=" + ControlSoftware.cia + " and usuario.cod_user=" + ControlSoftware.us)) {
            JOptionPane.showMessageDialog(null, "Usted no tiene acceso a modificar cuentas.");
            return;
        }
        u_cuenta l = new u_cuenta(""+Productos.getValueAt( Productos.getSelectedRow(),0));
        Dimension internalFrameSize = l.getSize();
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        l.setLocation((screenSize.width - internalFrameSize.width) / 2,
            (screenSize.height - internalFrameSize.height) / 2);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (!buscar.getText().isEmpty()) {
             bd.LlenarTabla("select IDBANCO AS COD, NOMBRE AS BANCO, FECHA_INGRESO AS FECHA_INGRESO from BANCO where cia=" + ControlSoftware.cia+" and nombre like '%"+buscar.getText()+"%'", Productos);
            
           } else {
            bd.LlenarTabla("select IDBANCO AS COD, NOMBRE AS BANCO, FECHA_INGRESO AS FECHA_INGRESO from BANCO where cia=" + ControlSoftware.cia, Productos);
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(mantenimiento_cuentas.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new mantenimiento_cuentas().setVisible(true);
            }
        });
    }
