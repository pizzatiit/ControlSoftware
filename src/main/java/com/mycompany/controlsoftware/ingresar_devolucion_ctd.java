 ...
 * @author Usuario
 */
public class ingresar_devolucion_ctd extends javax.swing.JInternalFrame {
    ...
    public ingresar_devolucion_ctd() {
        ...
        if (bd.valor("select max(cast(d.n_devolucion as unsigned))+1 as maximo from devoluciones d inner join factura f on d.id_factura=f.idfactura where d.cia=" + ControlSoftware.cia + " and f.tipo=1 and f.condicionpago=1") == null) {
            nc_fiscal.setText("1");
        } else {
            nc_fiscal.setText(String.format("%.0f", (Double.parseDouble(bd.valor("select max(cast(d.n_devolucion as unsigned))+1 as maximo from devoluciones d inner join factura f on d.id_factura=f.idfactura where d.cia=" + ControlSoftware.cia + " and f.tipo=1 and f.condicionpago=1")))));
        }
    ...
    private void initComponents() {
        ...
        jLabel3.setText("INGRESAR DEVOLUCIÓN CTD");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        */
        ...
        if(bd.valor("select d.n_devolucion from devoluciones d inner join factura f on f.idfactura=d.id_factura where f.tipo="+tipo+" and f.condicionpago=1").equals(nc_fiscal.getText())){
        ...
        if (opc == 0) {
            ...
            String v = bd.doble("INSERT INTO `recibopago`(`numero_recibo`, `tipo`, `fecha`, `descripcion`, `idcliente`, `idvendedor`, `cia`, `total`, `fechaingreso`, `estado`) VALUES ('" + nc_fiscal.getText() + "','" + typ + "','" + Date_Format.format(fecha.getDate()) + "','" + "Nota de crédito, "+obsdevolucion.getText() + ", devolucion de dinero ','" + cliente + "','" + vendedor + "','" + ControlSoftware.cia + "','" + total_devolucion + "','" + date.format(formatter) + "','1')", "SELECT @@IDENTITY AS 'id'");
            ...
            bd.EjecutarConsultas("INSERT INTO `detalle_recibopago`(`idrecibo`, `idfactura`, `abono`, `saldoanterior`, `nuevosaldo`, `cia`, `estado`, `fechasistema`) VALUES ('" + v + "','" + idfact.getText() + "','" + total_devolucion + "','" + total_devolucion + "','" + 0.00 + "','" + ControlSoftware.cia + "','1','" + date.format(formatter) + "')");
            bd.EjecutarConsultas("INSERT INTO `recibopago_datos` (`idrecibo`, `tipo`, `referencia`, `id_banco`, `id_cuenta`, `observacion`, `estado`, `cia`, `monto`, `fecha`, `fecha_sistema`) VALUES ( "+v+", '1000', 'EFECTIVO', '0', '0', 'DEVOLUCION DE DINERO', '1', "+ControlSoftware.cia+", '"+total_devolucion+"', '" + date.format(formatter) + "', '"+LocalDateTime.now()+"')");
            dispose();
            mantenimiento_devoluciones_ctd l1 = new mantenimiento_devoluciones_ctd();
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        seleccionarfactura_devolucion_ctd l1 = new seleccionarfactura_devolucion_ctd();
        Dimension internalFrameSize = l1.getSize();
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        l1.setLocation((screenSize.width - internalFrameSize.width) / 2,
            (screenSize.height - internalFrameSize.height) / 2);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ingresar_devolucion_ctd.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ingresar_devolucion_ctd().setVisible(true);
            }
        });
    }
